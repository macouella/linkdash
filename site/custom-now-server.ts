/**
 * A customised version of @now/node for local serverless testing.
 */
import {
  NowRequest,
  NowRequestBody,
...
export function sendError(res: NowResponse, statusCode: number, message: string) {
  res.statusCode = statusCode;
...
export function createServerWithHelpers(
  ...
  const server = new Server(async (_req, _res) => {
    ...
    setLazyProp<NowRequestBody>(req, "body", getBodyParser(req, req.body));
